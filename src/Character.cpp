...
Character::Character(string name, int width, int height, int zindex,
...
bool Character::load(SDL_Renderer* render) {
	...
	Sprite* spriteWalk = new Sprite(this->name + WALK_SUFFIX,
			this->imagePath + "/UMK3_Sub-Zero_walk.png", renderer, 66, 132, 10);
	Sprite* spriteStance = new Sprite(this->name + STANCE_SUFFIX,
			this->imagePath + "UMK3_Sub-Zero_stance.png", renderer, 66, 132, 6);
	Sprite* spriteJump = new Sprite(this->name + JUMP_SUFFIX,
			this->imagePath + "UMK3_Sub-Zero_duckjump.png", renderer, 66, 132,
			1);
	//TODO: Files path must be generated depending on the character
	this->characterSprites.insert(
			std::map<std::string, Sprite*>::value_type(this->name + WALK_SUFFIX,
					spriteWalk));
	this->characterSprites.insert(
			std::map<std::string, Sprite*>::value_type(
					this->name + STANCE_SUFFIX, spriteStance));
	this->characterSprites.insert(
			std::map<std::string, Sprite*>::value_type(this->name + JUMP_SUFFIX,
					spriteJump));
...
void Character::render(SDL_Renderer* render) {
	...
	TextureManager::Instance()->drawFrame(currentSprite->getSpriteId(),
			(int) positionX, (int) positionY, currentSprite->getSpriteWidth(),
			currentSprite->getSpriteHeight(), 1, currentFrame, this->renderer,
			SDL_FLIP_NONE);
}
...
void Character::draw() {
	...
	TextureManager::Instance()->drawFrame(currentSprite->getSpriteId(),
			(int) positionX, (int) positionY, currentSprite->getSpriteWidth(),
			currentSprite->getSpriteHeight(), 1, currentFrame, renderer,
			SDL_FLIP_NONE);
}
