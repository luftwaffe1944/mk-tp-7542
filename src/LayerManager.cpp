...
void LayerManager::refresh() {
	...
	for (unsigned int i = 0; i < this->characters.size() ; i++) {
		...
		if ( ( (windowWidth - (posXCharacter + characterWidth )) < WINDOW_MARGIN) && !layerReachedStageLimit( windowWidth, true) && isCharMovingRight && !passiveCharacter->reachedWindowLeftLimit() ) {
			refresh = true;
			rightOrientation = true;
			orientation = 1;
		}
		else if  (( posXCharacter < WINDOW_MARGIN ) && !layerReachedStageLimit( windowWidth, false) && isCharMovingLeft && !passiveCharacter->reachedWindowRightLimit())  {
			refresh = true;
